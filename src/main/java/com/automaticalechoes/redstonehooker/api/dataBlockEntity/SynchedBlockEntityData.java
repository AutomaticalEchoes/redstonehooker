...
public class SynchedBlockEntityData {
   ...
   private static final Object2IntMap<Class<? extends DataBlockEntity>> ENTITY_ID_POOL = new Object2IntOpenHashMap<>();
   private static final int EOF_MARKER = 255;
   private static final int MAX_ID_VALUE = 254;
   private final DataBlockEntity entity;
   private final Int2ObjectMap<DataItem<?>> itemsById = new Int2ObjectOpenHashMap<>();
   private final ReadWriteLock lock = new ReentrantReadWriteLock();
   private boolean isEmpty = true;
   private boolean isDirty;

   public SynchedBlockEntityData(DataBlockEntity p_135351_) {
      this.entity = p_135351_;
   }

   public static <T> EntityDataAccessor<T> defineId(Class<? extends DataBlockEntity> p_135354_, EntityDataSerializer<T> p_135355_) {
         ...
         try {
            ...
            if (!oclass.equals(p_135354_)) {
               ...
               if (LOGGER.isDebugEnabled())
                  LOGGER.warn("defineId called for: {} from {}", p_135354_, oclass, new RuntimeException());
      ...
      if (ENTITY_ID_POOL.containsKey(p_135354_)) {
         ...
         while (oclass1 != BlockEntity.class) {
   ...
   public <T> void set(EntityDataAccessor<T> p_135382_, T p_135383_) {
      DataItem<T> dataitem = this.getItem(p_135382_);
   ...
   public static void pack(@Nullable List<DataItem<?>> p_135359_, FriendlyByteBuf p_135360_) {
      if (p_135359_ != null) {
         for (DataItem<?> dataitem : p_135359_) {
   ...
   private static <T> void writeDataItem(FriendlyByteBuf p_135368_, DataItem<T> p_135369_) {
   ...
   @Nullable
   public static List<DataItem<?>> unpack(FriendlyByteBuf p_135362_) {
      List<DataItem<?>> list = null;
   ...
   private static <T> DataItem<T> genericHelper(FriendlyByteBuf p_135364_, int p_135365_, EntityDataSerializer<T> p_135366_) {
      return new DataItem<>(p_135366_.createAccessor(p_135365_), p_135366_.read(p_135364_));
   }

   public void assignValues(List<DataItem<?>> p_135357_) {
      this.lock.writeLock().lock();

      try {
         for (DataItem<?> dataitem : p_135357_) {
            DataItem<?> dataitem1 = this.itemsById.get(dataitem.getAccessor().getId());
            if (dataitem1 != null) {
   ...
   private <T> void assignValue(DataItem<T> p_135376_, DataItem<?> p_135377_) {
